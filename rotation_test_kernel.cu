...
int main()
{
	...
	img = read_BMP_opencv("input_images/src2048.bmp", f_width, f_height);
	data = img.data;
	

	cudaMalloc((void**)&d_data, sizeof(uchar)*f_width*f_height);
	cudaMalloc((void**)&d_rotated, sizeof(uchar)*f_width*f_height);
	cudaMallocHost((void**)&h_result, sizeof(uchar)*f_width*f_height);
	...
	rotateImage << <numBlocks, threadsPerBlock>> > (d_data, d_rotated, sine, cosine, xCenter, yCenter, f_width, f_height);
	cudaMemcpy(h_result, d_rotated, sizeof(uchar)*f_width*f_height, cudaMemcpyDeviceToHost);
	cudaDeviceSynchronize();
	cudaEventRecord(stop, 0);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&elapsedTime, start, stop);

	printf("Rotation Time: %3.1f ms\n", elapsedTime);
